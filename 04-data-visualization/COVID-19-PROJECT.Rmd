...
ggplot(confirmed_cases_worldwide, aes(x=date, y= cum_cases)) +
  geom_line() +
  ylab("Cumulative confirmed cases by date")
```
...
```{r}
# Step 1: Debugging the date column in confirmed_cases_china_vs_world
print("Debugging confirmed_cases_china_vs_world")
str(confirmed_cases_china_vs_world)  # Check the structure of the dataset
head(confirmed_cases_china_vs_world) # Check the first few rows of data
```


```{r}
# Step 2: Debugging the date column in who_events
print("Debugging who_events")
str(who_events)  # Check the structure of the dataset
head(who_events) # Check the first few rows of data
```


```{r}
# Ensure that the date column is of Date type in the dataset
confirmed_cases_china_vs_world <- confirmed_cases_china_vs_world %>%
  mutate(date = as.Date(date))
```


```{r}
# Ensure the date column in who_events is also Date type (you already did this)
who_events <- tribble(
  ~ date, ~ event,
...
```{r}
# Step 3: Plot the data with annotations for important events, including scale_x_date for date formatting
# Plot the data with annotations for important events, ensuring scale_x_date is used
plt_cum_confirmed_cases_china_vs_world +
  geom_vline(aes(xintercept = date), data = who_events, linetype = "dashed") +  # Convert date to numeric
